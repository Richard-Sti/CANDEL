...
def plot_h0_histogram(field_rows, stack, field_stat, out_pdf):
...
def plot_h0_vs_evidence(field_rows, out_pdf):
    ...
    with plt.style.context("science"):
        ...
        fig.suptitle(f"CH0 single Manticore fields; {len(field_rows)} fields")
        fig.savefig(out_pdf, dpi=FIGURE_DPI, bbox_inches="tight")
        out_png = out_pdf.with_suffix(".png")
        fig.savefig(out_png, dpi=FIGURE_DPI, bbox_inches="tight")
        plt.close(fig)
...
def main():
    ...
    hist_png = plot_h0_histogram(field_rows, stack, args.field_stat, hist_pdf)
    ...
    evidence_png = plot_h0_vs_evidence(field_rows, evidence_pdf)
