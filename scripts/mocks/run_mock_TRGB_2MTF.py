#!/usr/bin/env python
"""Mock closure tests for combined TRGB + 2MTF model.
...
Usage:
    ...
    python run_mock_TRGB_2MTF.py --n-mocks 5 --n-procs 5 --use-field
"""
import argparse
import os
import sys
...
from candel.mock import gen_TRGB_2MTF_mock  # noqa: E402
from candel.mock.TRGB_2MTF_mock import (DEFAULT_ANCHORS,  # noqa: E402
                                         DEFAULT_TRUE_PARAMS)
...
def make_mock_config(seed, num_warmup=500, num_samples=500,
                     ...
                     use_field=False, rmax=40.0,
                     mag_lim=None, mag_lim_width=None,
                     infer_selection=True):
    ...
    config["model"]["which_selection"] = "TRGB_magnitude"
...
def run_one_mock(args):
    ...
    (seed, true_params, mock_kwargs, fix_H0, fix_a_TFR,
     num_warmup, num_samples, use_field, infer_selection) = args

    try:
        tp = {**DEFAULT_TRUE_PARAMS, **(true_params or {})}
        data, tp_out, n_parent = gen_TRGB_2MTF_mock(
        ...
        # Convert Vext to galactic for comparison
        ...
        try:
            model = candel.model.TRGB2MTFModel(tmp, data)
...
def run_batch(n_mocks, n_procs, true_params, mock_kwargs,
    ...
    args_list = [
        (seed, true_params, mock_kwargs, fix_H0, fix_a_TFR,
         num_warmup, num_samples, use_field, infer_selection)
    ...
    tag = "_field" if use_field else ""
    outfile = os.path.join(outdir, f"mock_TRGB_2MTF_biases_{mode}{tag}.npz")
...
def run_single(seed, true_params, mock_kwargs, fix_H0, fix_a_TFR,
    ...
    # Convert Vext to galactic coordinates
    Vx, Vy, Vz = tp_out["Vext_x"], tp_out["Vext_y"], tp_out["Vext_z"]
    Vmag, Vell, Vb = radec_cartesian_to_galactic(
        np.array([Vx]), np.array([Vy]), np.array([Vz]))
    tp_out["Vext_mag"] = float(Vmag)
    ...
    n = len(data["mag_obs"])
    print(f"\nMock TRGB+2MTF catalog: {n} hosts (parent: {n_parent})")
    print(f"mag_obs (TRGB): [{data['mag_obs'].min():.2f}, "
          f"{data['mag_obs'].max():.2f}]")
    print(f"mag_TFR:        [{data['mag_TFR'].min():.2f}, "
          f"{data['mag_TFR'].max():.2f}]")
    print(f"eta:            [{data['eta'].min():.3f}, "
          f"{data['eta'].max():.3f}]")
    print(f"czcmb:          [{data['czcmb'].min():.0f}, "
          f"{data['czcmb'].max():.0f}] km/s")
...
def main():
    ...
    parser.add_argument("--infer-selection", action="store_true",
                        help="Infer selection thresholds")
    ...
    if args.single:
        run_single(args.seed, true_params, mock_kwargs,
                   ...
                   args.use_field, args.infer_selection)
    else:
