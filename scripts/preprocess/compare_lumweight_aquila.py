...
cell = BOXSIZE / NGRID
obs = BOXSIZE / 2
xg = r_mpc * np.cos(gal_b) * np.cos(gal_l) + obs
yg = r_mpc * np.cos(gal_b) * np.sin(gal_l) + obs
zg = r_mpc * np.sin(gal_b) + obs

# CIC deposit function
def deposit_cic(weights):
...
rho_stored = deposit_cic(w_stored)
print("Depositing computed weights...")
rho_computed = deposit_cic(w_computed)
...
v_carrick = np.load('data/Carrick_reconstruction_2015/vField_0Runs.npy')
...
for i_cl in range(n_clusters):
    ...
    # Interpolate density from all three sources
...
for i, idx in enumerate(sample_idx):
    ax = axes[i]
    # Carrick LOS file stores 1+δ already, our interpolated values are δ
    ax.plot(los_r, los_density_carrick[idx], 'g-', lw=2, label='Carrick LOS file')
    ax.plot(los_r, 1 + los_density_stored[idx], 'b-', lw=2, alpha=0.8, label='Aquila lumWeight')
    ax.plot(los_r, 1 + los_density_computed[idx], 'r--', lw=2, alpha=0.8, label='My weights')
...
for i, idx in enumerate(sample_idx):
    ...
    ax.plot(los_r, los_velocity_stored[idx], 'b-', lw=2, alpha=0.8, label='Aquila lumWeight')
    ax.plot(los_r, los_velocity_computed[idx], 'r--', lw=2, alpha=0.8, label='My weights')
    ax.axhline(0, color='k', ls=':', alpha=0.5)
    ax.set_xlabel('r [Mpc/h]')
