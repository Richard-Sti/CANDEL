...
DEFAULT_RSD_COMPARISON_PLOT_SCRIPT = Path(
    __file__).with_name("plot_rsd_comparison.py")
DEFAULT_RSD_CROSS_SCRIPT = Path(__file__).with_name(
    "compute_rsd_pylians_cross_correlation.py")
...
def add_common_args(parser: argparse.ArgumentParser) -> None:
    ...
    parser.add_argument(
        "--state",
        help="State directory name, e.g. state_6124.")
    parser.add_argument(
        "--output-root",
        type=Path,
        help="Output root. Defaults to <run-root>/forward inferred from the MCMC path.",  # noqa: E501
    )
    parser.add_argument(
        "--borg-forward",
        type=Path,
        default=DEFAULT_BORG_FORWARD)
    parser.add_argument("--mpirun", default="mpirun")
    parser.add_argument(
        "--mpi-launcher",
        default=os.environ.get("BORG_MPI_LAUNCHER"),
        help="Launcher template with {nprocs}, e.g. 'srun -u -n {nprocs} --mpi=pmix'.",  # noqa: E501
    )
    parser.add_argument(
        "--nprocs",
        type=int,
    ...
    parser.add_argument(
        ...
        help=f"Override [gravity_chain_2] pm_nsteps in generated configs. Default: {DEFAULT_PM_NSTEPS}.",  # noqa: E501
    )
    ...
    parser.add_argument(
        ...
        action="store_false",
        help="Do not create a local MCMC copy with /scalars/BORG_vobs=[0,0,0].",  # noqa: E501
    )
...
def parse_args() -> argparse.Namespace:
    ...
    mode.add_argument(
        "--rsd",
        action="store_true",
        help="Write redshift-space density.")
    mode.add_argument(
        "--real-space",
        dest="rsd",
        action="store_false",
        help="Write real-space density.")
    ...
    run.add_argument(
        ...
        help=(
            ...
            "[borg_fields.chains.<name>].field_output_dir from local_config.toml."),  # noqa: E501
    )
    run.add_argument(
        "--no-single-output",
        action="store_true",
        help="Leave only the split rank files; do not pack a single HDF5 product.",  # noqa: E501
    )
    run.add_argument(
        "--sph-fields",
        dest="sph_fields",
    ...
    run.add_argument(
        "--cosmotool-sph",
        type=Path,
        default=DEFAULT_COSMOTOOL_SPH)
    run.add_argument(
        "--sph-resolution",
        type=int,
        help="Grid resolution for SPH, CIC, or PCS. Default: /<mode>/scalars/N0.",  # noqa: E501
    )
    run.add_argument(
        "--cic-chunk-size",
        type=int,
        default=1_000_000,
        help="Particle chunk size for Pylians CIC/PCS gridding. Default: 1000000.",  # noqa: E501
    )
    run.add_argument(
        "--sph-radius-limit",
        type=float,
        help="SPH spherical radius limit. Default: boxsize, which includes the full cube.",  # noqa: E501
    )
    run.add_argument(
        "--sph-periodic",
        type=int,
    ...
    run.add_argument(
        "--sph-threads",
        type=int,
        default=int(os.environ.get("SPH_OMP_THREADS", os.environ.get("NPROCS", "8"))),  # noqa: E501
        help="OpenMP threads for CosmoTool SPH. Default: SPH_OMP_THREADS or NPROCS.",  # noqa: E501
    )
    run.add_argument(
        "--keep-sph-work",
        action="store_true",
    ...
    run.add_argument(
        "--keep-particles",
        action="store_true",
        help="Keep /<mode>/u_pos and /<mode>/u_vel in the packed product; disables final product slimming.",  # noqa: E501
    )
    ...
    run.add_argument(
        "--plot-slice-index",
        type=int,
        help="Default: middle slice.")
    run.set_defaults(sph_fields=True, plots=False)
    add_common_args(run)

    validate = subparsers.add_parser(
        "validate-rsd",
        help="Choose random MCMC sample(s), run RSD, and compare to /scalars/BORG_final_density.",  # noqa: E501
    )
    validate.add_argument(
        "chain_dir",
        type=Path,
        help="Directory containing mcmc_*.h5 files.")
    validate.add_argument(
        "--samples",
        type=int,
        default=1,
        help="Number of random samples. Default: 1.")
    validate.add_argument(
        "--seed",
        type=int,
        default=12345,
    ...
    validate.add_argument(
        "--glob",
        default="mcmc_*.h5",
        help="MCMC filename glob. Default: mcmc_*.h5.")
    ...
    validate.add_argument(
        ...
        action="store_false",
        help="Do not write a generated/reference/difference density slice plot.",  # noqa: E501
    )
    validate.add_argument(
        "--rsd-comparison-plot-dir",
        type=Path,
        help="Default: RSD output directory/plots.")
    validate.add_argument(
        "--rsd-comparison-plot-axis",
        type=int,
        choices=(
            0,
            1,
        ...
        default=2)
    validate.add_argument(
        "--rsd-comparison-plot-index",
        type=int,
        help="Default: middle slice.")
    validate.add_argument(
        "--plot-python",
        type=Path,
        default=DEFAULT_PLOT_PYTHON)
    validate.add_argument(
        "--rsd-comparison-plot-script",
        type=Path,
        default=DEFAULT_RSD_COMPARISON_PLOT_SCRIPT)
    validate.add_argument(
        "--no-rsd-cross-correlation",
        dest="rsd_cross_correlation",
        action="store_false",
        help="Do not compute the Pylians RSD cross-correlation against /scalars/BORG_final_density.",  # noqa: E501
    )
    validate.add_argument(
        "--rsd-cross-output-dir",
        type=Path,
        help="Default: RSD output directory/plots.")
    validate.add_argument(
        "--rsd-cross-boxsize",
        type=float,
        default=681.0,
        help="Box size in Mpc/h. Default: 681.")
    validate.add_argument(
        "--rsd-cross-axis",
        type=int,
        default=0,
        help="Pylians line-of-sight axis. Default: 0.")
    validate.add_argument(
        "--rsd-cross-threads",
        type=int,
        default=4,
        help="Pylians thread count. Default: 4.")
    validate.add_argument(
        "--rsd-cross-kmax",
        type=float,
        default=0.2,
        help="Maximum k for the mean-r validation statistic. Default: 0.2 h/Mpc.",  # noqa: E501
    )
    validate.add_argument(
        "--rsd-cross-min-mean-r",
        type=float,
        default=0.99,
        help="Warn if mean r(k) up to --rsd-cross-kmax is below this value. Default: 0.99.",  # noqa: E501
    )
    validate.add_argument(
        "--rsd-cross-script",
        type=Path,
        default=DEFAULT_RSD_CROSS_SCRIPT)
...
def infer_layout(mcmc: Path, output_root: Path |
                 None) -> tuple[Path, str, Path]:
    chain_dir = mcmc.parent
    if output_root is not None:
        return output_root.expanduser().resolve(), chain_dir.name, chain_dir
    if chain_dir.parent.name != "chain":
        raise ValueError(
            "Expected <run-root>/chain/<subchain>/mcmc_*.h5; pass --output-root.")  # noqa: E501
...
def find_params(
    ...
    if not candidates:
        raise FileNotFoundError(
            f"No state_*/params.ini found under {chain_dir}")
    if len(candidates) > 1:
        print(f"Using template params.ini: {candidates[0]}", flush=True)
        print(
            "Pass --params or --state to choose a different state config.",
            flush=True)
...
def patch_mcmc_missing_vobs(mcmc: Path, out_dir: Path, create: bool) -> Path:
    ...
    if not create:
        return patched
    if patched.is_file() and patched.stat(
    ).st_mtime >= mcmc.stat().st_mtime and has_borg_vobs(patched):
        return patched

    patched.parent.mkdir(parents=True, exist_ok=True)
    shutil.copy2(mcmc, patched)
    with h5py.File(patched, "a") as handle:
        handle.require_group("scalars").create_dataset(
            "BORG_vobs", data=[0.0, 0.0, 0.0])
    return patched
...
def parse_mas_values(values: list[str]) -> list[str]:
    ...
    if not normalised:
        raise ValueError(
            "--mas must name at least one mass-assignment scheme.")
    return normalised
...
def run_forward(mcmc: Path, args: argparse.Namespace,
                do_rsd: bool) -> tuple[Path, int, Path]:
    ...
    mcmc_for_borg = patch_mcmc_missing_vobs(
        mcmc, out_dir, create=not args.dry_run) if args.patch_missing_vobs else mcmc  # noqa: E501
    ...
    write_run_config(
        ...
        config,
        do_rsd=do_rsd,
        console_output=log_dir /
        "borg_forward",
        pm_nsteps=args.pm_nsteps)
    ...
    print(
        f"Split output files: {output}_0 ... {output}_{
            args.nprocs - 1}",
        flush=True)
...
def pack_split_outputs(
    ...
    with h5py.File(rank_paths[0], "r") as first, h5py.File(product, "a") as out:  # noqa: E501
        ...
        density_shape = (sum(density_slabs),) + \
            first["final_density"].shape[1:]
        ...
        density = group.create_dataset("final_density",
                                       density_shape,
                                       dtype=first["final_density"].dtype,
                                       chunks=(1,
                                               ) + density_shape[1:])
        shat = group.create_dataset("s_hat_field",
                                    shat_shape,
                                    dtype=first["s_hat_field"].dtype,
                                    chunks=(1,
                                            ) + shat_shape[1:])
        if "v_field" in first:
        ...
        for path in rank_paths:
            with h5py.File(path, "r") as handle:
                ...
                if n_density:
                    density[density_offset: density_offset +
                            n_density] = handle["final_density"][...]
                    density_offset += n_density

                n_shat = handle["s_hat_field"].shape[0]
                if n_shat:
                    shat[shat_offset: shat_offset +
                         n_shat] = handle["s_hat_field"][...]
                    shat_offset += n_shat

                n_particle = handle["u_pos"].shape[0] if has_particles else 0
                if n_particle and pos is not None and vel is not None:
                    pos[particle_offset: particle_offset +
                        n_particle] = handle["u_pos"][...]
                    vel[particle_offset: particle_offset +
                        n_particle] = handle["u_vel"][...]
...
def borg_velocity_multiplier(group) -> float:
    # BORG writes /u_vel in internal units. Its gridded velocity helpers convert  # noqa: E501
    # with ParticleBasedForwardModel::getVelocityMultiplier() = unit_v0 /
    # a_final.
    unit_v0 = 100.0
    a_final = 1.0
...
def borg_mcmc_metadata(mcmc: Path) -> dict[str, object]:
    ...
    with h5py.File(mcmc, "r") as handle:
        if "scalars" not in handle:
            metadata["mcmc_metadata_status"] = "source_mcmc_has_no_scalars_group"  # noqa: E501
            return metadata
...
def borg_forward_metadata(group) -> dict[str, object]:
    ...
    if all(length is not None for length in lengths):
        ...
        metadata["observer_position_convention"] = (
            "grid coordinates in Mpc/h, origin at the lower corner of the stored field"  # noqa: E501
        )
        ...
        if all(corner is not None for corner in corners):
            ...
            metadata["observer_position_borg_coordinates"] = corner_values + \
                0.5 * box_lengths
...
def borg_field_metadata(src, group_name: str) -> dict[str, object]:
    ...
    source_mcmc_value = group.attrs.get(
        "source_mcmc", src.attrs.get(
            "source_mcmc", ""))
    source_mcmc_text = source_mcmc_value.decode() if isinstance(
        source_mcmc_value, bytes) else str(source_mcmc_value)
    metadata = borg_mcmc_metadata(
        Path(source_mcmc_text)) if source_mcmc_text else {}
...
def write_velocity_component_views(fields) -> None:
    ...
    if velocity.ndim != 4 or velocity.shape[-1] != 3:
        raise ValueError(
            f"Expected velocity shape (N, N, N, 3), got {
                velocity.shape}")
    ...
    for axis, name in enumerate(("vx", "vy", "vz")):
        layout = h5py.VirtualLayout(
            shape=velocity.shape[:-1], dtype=velocity.dtype)
...
def dump_sph_particles(
        product: Path,
        group_name: str,
        particles: Path) -> float:
    ...
    with h5py.File(product, "r") as src, h5py.File(particles, "w") as out:
        ...
        if pos.shape != vel.shape or pos.shape[1] != 3:
            raise ValueError(
                f"Expected matching (N, 3) u_pos/u_vel in /{group_name}")
...
def write_sph_product(raw_sph: Path,
                      product: Path,
                      group_name: str,
                      attrs: dict[str,
                                  float | int | str]) -> None:
    with h5py.File(raw_sph, "r") as src, h5py.File(product, "a") as out:
        ...
        if "velocity_multiplier_internal_to_kms" in attrs:
            sph.attrs["velocity_multiplier_internal_to_kms"] = attrs["velocity_multiplier_internal_to_kms"]  # noqa: E501
        ...
        if "velocity_multiplier_internal_to_kms" in attrs:
            velocity.attrs["velocity_multiplier_internal_to_kms"] = attrs["velocity_multiplier_internal_to_kms"]  # noqa: E501
        nonzero = density != 0.0
        for axis in range(3):
        ...
        sph.create_dataset("num_in_cell",
                           data=src["num_in_cell"][...],
                           chunks=(1,
                                   ) + density.shape[1:])
...
def run_sph_fields(
    ...
    with h5py.File(product, "r") as handle:
        ...
        if any(abs(length - boxsize) > 1e-6 for length in lengths):
            raise ValueError(
                f"SPH gridding assumes a cubic box; got L0,L1,L2={lengths}")

    radius_limit = args.sph_radius_limit if args.sph_radius_limit is not None else boxsize  # noqa: E501
...
def run_pylians_mas_fields(
    ...
    with h5py.File(product, "r") as handle:
        ...
        if any(abs(length - boxsize) > 1e-6 for length in lengths):
            raise ValueError(
                f"{mas.upper()} gridding assumes a cubic box; got L0,L1,L2={lengths}")  # noqa: E501
        n_particles = group["u_pos"].shape[0]
        velocity_multiplier = borg_velocity_multiplier(group)

    mas_name = pylians_mas_name(mas)
    print(
        f"Running Pylians periodic {mas_name} gridding: resolution={resolution}, "  # noqa: E501
    ...
    with h5py.File(product, "r") as handle:
        ...
        for start in range(0, n_particles, chunk_size):
            ...
            positions = centered_positions_to_pylians(
                pos[start:stop], boxsize, resolution)
            velocity = (vel[start:stop] *
                        velocity_multiplier).astype("f4", copy=False)
            ...
            for axis in range(3):
                weights = np.ascontiguousarray(
                    velocity[:, axis], dtype=np.float32)
                MASL.MA(
                    positions,
                    momentum[axis],
                    np.float32(boxsize),
            ...
            print(
                f"{mas_name} deposited particles {stop} / {n_particles}",
                flush=True)
    ...
    with h5py.File(product, "a") as handle:
        ...
        fields.attrs["velocity_multiplier_internal_to_kms"] = velocity_multiplier  # noqa: E501
        fields.attrs["grid_cell_size"] = boxsize / resolution
        fields.attrs["position_convention"] = (
            "BORG centred positions shifted by boxsize/2 - cell_size/2 before Pylians deposition"  # noqa: E501
        )
        ...
        write_generic_field_schema(
            fields, borg_field_metadata(
                handle, group_name))

    print(
        f"Wrote {mas_name} fields to: {product}:/{group_name}/{mas}",
        flush=True)
...
def run_borg_cic_fields(
    ...
    with h5py.File(product, "a") as handle:
        ...
        if "v_field" not in group:
            raise KeyError(
                f"Missing /{group_name}/v_field in {product}; BORG must be run with --vfield")  # noqa: E501

        density = group["final_density"]
        vfield = group["v_field"]
        if vfield.ndim != 4:
            raise ValueError(
                f"Expected /{group_name}/v_field to be 4D, got {vfield.shape}")
        ...
        if raw_velocity.shape[-1] == 3:
            ...
            raise ValueError(
                f"Expected /{group_name}/v_field component axis of length 3, got {vfield.shape}")  # noqa: E501
    ...
    print(
        f"Wrote BORG CIC fields to: {product}:/{group_name}/cic-borg",
        flush=True)
...
def remove_particle_datasets(product: Path, group_name: str) -> None:
    ...
    if removed:
        print(
            f"Removed particle datasets from final product: {
                ', '.join(removed)}",
            flush=True)
...
def cleanup_forward_workdirs(
    ...
    removed = []
    prune_from = []
    cleanup_root = Path(os.path.commonpath(
        [str(product), *(str(path.resolve()) for path in work_dirs)]))
    for work_dir in work_dirs:
        work_dir = work_dir.resolve()
        if product == work_dir or work_dir in product.parents:
            print(
                f"[WARN] Skipping cleanup for work directory containing product: {work_dir}",  # noqa: E501
                flush=True)
            continue
        if work_dir.exists():
            if preserve_names:
                ...
                if removed_children:
                    print(
                        f"Removed BORG forward generated files from {work_dir}: " +  # noqa: E501
                        ", ".join(
                            path.name for path in removed_children),
                        flush=True,
                    )
    ...
    for parent in prune_from:
        ...
        while parent != cleanup_root and product != parent and parent not in product.parents:  # noqa: E501
            try:
                parent.rmdir()
            except OSError:
                break
...
def slim_field_product(product: Path, group_name: str, mas: str) -> None:
    ...
    with h5py.File(product, "r") as src, h5py.File(tmp_product, "w") as out:
        ...
        if overdensity.dtype != np.dtype("float32"):
            raise TypeError(
                f"Expected /{group_name}/{mas}/overdensity to be float32, got {overdensity.dtype}")  # noqa: E501
        if velocity.dtype != np.dtype("float32"):
            raise TypeError(
                f"Expected /{group_name}/{mas}/velocity to be float32, got {velocity.dtype}")  # noqa: E501
    ...
    print(
        f"Slimmed final product to: {product}:/overdensity, /velocity, /vx, /vy, and /vz",  # noqa: E501
        flush=True)


def run_gridded_fields(
        product: Path,
        group_name: str,
        args: argparse.Namespace,
        mas: str) -> None:
        ...
        with h5py.File(rank_output, "r") as handle:
            ...
            if require_vfield and rank == 0:
                ...
                if vfield.shape[0] != 3 and vfield.shape[-1] != 3:
                    raise ValueError(
                        f"Expected /v_field component axis of length 3, got {vfield.shape}")  # noqa: E501
            rank_n0 = int(handle["scalars/N0"][0])
            expected_n0 = rank_n0 if expected_n0 is None else expected_n0
            if rank_n0 != expected_n0:
                raise ValueError(
                    f"Inconsistent N0 in {rank_output}: {rank_n0} != {expected_n0}")  # noqa: E501

    if density_slabs != expected_n0:
        raise ValueError(
            f"Split density slabs sum to {density_slabs}, expected {expected_n0}")  # noqa: E501
    if require_particles and particle_count != velocity_count:
        raise ValueError(
            f"u_pos count {particle_count} != u_vel count {velocity_count}")
    particle_text = f", {particle_count} particles" if require_particles else ""  # noqa: E501
    print(
...
def compare_to_mcmc(mcmc: Path,
    ...
    with h5py.File(mcmc, "r") as ref_handle:
        ...
        if offset != ref.shape[0]:
            raise ValueError(
                f"Compared {offset} slabs along axis 0, expected {
                    ref.shape[0]}")

    rms = float(np.sqrt(sum_sq / count)) if count else 0.0
    threshold = atol + rtol * max_ref_abs
    print(
        f"RSD reference comparison: max_abs_diff={
            max_abs:.16e}, rms_diff={
            rms:.16e}",
        flush=True)
    print(
        f"RSD reference tolerance: {
            threshold:.16e} (atol={atol}, rtol={rtol})",
        flush=True)
...
def write_rsd_comparison_plot(
    ...
    plot_python = require_file_preserve_symlink(
        args.plot_python, "plot Python")
    plot_script = require_file(
        args.rsd_comparison_plot_script,
        "RSD comparison plot script")
    plot_dir = (
    ...
    index_label = str(
        args.rsd_comparison_plot_index) if args.rsd_comparison_plot_index is not None else "mid"  # noqa: E501
    plot_path = plot_dir / f"rsd_comparison_mcmc_{iteration}_axis{
        args.rsd_comparison_plot_axis}_slice{index_label}.png"
    cmd = [
        str(plot_python),
        str(plot_script),
        "--mcmc",
    ...
    try:
        ...
        print(
            f"[WARN] RSD comparison plot failed with exit code {
                exc.returncode}; continuing.",
            flush=True)
        return None
    return plot_path
...
def write_rsd_cross_correlation(
    ...
    plot_python = require_file_preserve_symlink(
        args.plot_python, "plot Python")
    cross_script = require_file(
        args.rsd_cross_script,
        "RSD Pylians cross-correlation script")
    output_dir = (
    ...
    try:
        ...
        print(
            f"[WARN] RSD Pylians cross-correlation failed with exit code {
                exc.returncode}; continuing.",
            flush=True)
        return None
    ...
    try:
        ...
        print(
            f"[WARN] Could not read RSD cross-correlation metrics from {metrics_json}: {exc}",  # noqa: E501
            flush=True)
        return None
    ...
    if metrics.get("passed"):
        ...
        print(
            "[WARN] RSD Pylians cross-correlation: FAIL; continuing.",
            flush=True)
    return metrics


def choose_mcmc_files(
        chain_dir: Path,
        glob: str,
        samples: int,
        seed: int) -> list[Path]:
...
def write_product_plots(
    ...
    expected = [
        output_dir / f"{product.stem} _{group} _{mas} _field_slices.png"
        for group in groups]
...
def check_plot_environment(args: argparse.Namespace) -> None:
    ...
    cmd = [
        ...
        ("import h5py, matplotlib, numpy; "
            "print('Plot Python OK:', matplotlib.__version__, numpy.__version__, h5py.__version__)"),  # noqa: E501
    ]
    env = os.environ.copy()
    env["PYTHONNOUSERSITE"] = "1"

    print("Checking plot Python before expensive field generation:", flush=True)  # noqa: E501
    print(" ".join(shlex.quote(part) for part in cmd), flush=True)
    subprocess.run(cmd, check=True, env=env)
    subprocess.run([str(plot_python), "-m", "py_compile",
                   str(plot_script)], check=True, env=env)
...
def print_run_summary(
    ...
    print(
        f"  Split outputs: {
            ', '.join(
                str(path) for path in split_dirs)}",
        flush=True)
    if product is not None:
        ...
        if args.sph_fields:
            if args.keep_particles:
                ...
                print(
                    "  Final product contains only field datasets: no; particles were kept",  # noqa: E501
                    flush=True)
            else:
                print(
                    "  Gridded fields: /overdensity, /velocity, /vx, /vy, and /vz",  # noqa: E501
                    flush=True)
                print(
                    "  Final product contains only field datasets: yes",
                    flush=True)
        else:
...
def main() -> None:
    ...
    if getattr(args, "command", None) == "run":
        ...
        if len(args.mas) > 1 and args.single_output is not None:
            raise ValueError(
                "--single-output can only be used with one --mas value.")
    if args.command == "run":
        ...
        output_pattern, iteration, out_dir = run_forward(
            mcmc, args, do_rsd=args.rsd)
        if args.dry_run:
            ...
            if not args.no_single_output:
                for mas, product in zip(args.mas, product_paths):
                    ...
                    if args.sph_fields:
                        ...
                        if args.keep_particles:
                            print(
                                f"{mas.upper()} fields would be written under: "  # noqa: E501
                            ...
                            print(
                                ...
                                f"{product}:/overdensity, /velocity, /vx, /vy, and /vz",  # noqa: E501
                                flush=True,
                            )
                        if mas == "sph":
                            print(
                                f"SPH OpenMP threads: {
                                    args.sph_threads}", flush=True)
                if args.sph_fields:
                    if args.keep_particles:
                        print(
                            "Particle datasets would be kept; final products would not be slimmed.",  # noqa: E501
                            flush=True)
                    else:
                        print(
                            "Final products will contain only the generic field datasets.",  # noqa: E501
                            flush=True)
            return
        if not args.dry_run and not args.no_single_output:
            ...
            split_dirs = [out_dir]
            if args.include_rsd and not args.rsd:
                rsd_pattern, rsd_iteration, rsd_out_dir = run_forward(
                    mcmc, args, do_rsd=True)
            ...
            for mas, product in zip(args.mas, product_paths):
                ...
                plot_paths: list[Path] = []
                if args.plots:
                    plot_paths = write_product_plots(
                        product, plotted_groups, args, mas)
                if args.sph_fields:
                    if args.keep_particles:
                        print(
                            f"Kept particle datasets in full product: {product}",  # noqa: E501
                            flush=True)
                    else:
                        if len(plotted_groups) != 1:
                            raise ValueError(
                                "Slim final products support exactly one gridded mode.")  # noqa: E501
    ...
    for mcmc in mcmcs:
        ...
        if not args.dry_run:
            ...
            if cross_metrics is not None:
                ...
                print(
                    f"  RSD cross-correlation plot: {cross_metrics['plot']} ",
                    flush=True)
                print(
                    f"  RSD cross-correlation CSV: {cross_metrics['csv']} ",
                    flush=True)
            status = "PASS" if metrics["passed"] else "FAIL"
            print(
                f"  Comparison: {status} max_abs={
                    metrics['max_abs']:.6e}, " f"rms={
                    metrics['rms']:.6e}, threshold={
                    metrics['threshold']:.6e}",
                flush=True,
            )
            if not metrics["passed"]:
                print(
                    "  Validation mismatch is non-fatal; continuing.",
                    flush=True)
            if cross_metrics is not None and not cross_metrics["passed"]:
                print(
                    "  Cross-correlation mismatch is non-fatal; continuing.",
                    flush=True)
            cleanup_forward_workdirs(
                [out_dir],
                mcmc.resolve(),
                preserve_names=(
                    "plots",
