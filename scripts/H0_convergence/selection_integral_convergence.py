...
---------------
  ...
  cz    P_sel = int N(cz_obs | cz_cosmo(r), sv)   Phi((cz_lim-cz_obs)/w) dcz_obs  # noqa: E501
...
For redshift selection:
  bias_proxy(R)       N_hosts * H0-spread(delta_logS) / dH0  [1/(km/s/Mpc)].
                      Multiply by sigma_H0 to get approx H0 bias in sigma units.  # noqa: E501
...
def print_bias_proxy(
    ...
    if selection == "mag":
        ...
        print("  [1/(km/s/Mpc);  bias_in_sigma ~= bias_proxy * sigma_H0]")
