...
while [[ $# -gt 0 ]]; do
    case "$1" in
        -h|--help)
            ...
            echo ""
            echo "Tests the Mode 1 brute-force phi marginaliser used by"
            echo "_eval_bruteforce_phi. Two checks per galaxy:"
            echo "  1. Grid convergence vs n_phi per sub-range (101 → 100001)."
            echo "  2. Config per-type ranges vs full [-π, π] integration:"
            echo "     if the per-type ranges miss likelihood mass, this shows"
            echo "     as a non-zero gap."
            echo ""
