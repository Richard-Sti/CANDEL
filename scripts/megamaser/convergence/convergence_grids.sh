...
while [[ $# -gt 0 ]]; do
    case "$1" in
        -h|--help)
            ...
            echo ""
            echo "Tests the Mode 2 adaptive per-spot r-grid + shared trapezoidal"
            echo "phi grid, for circular physics. Two sweeps per galaxy:"
            echo "  1. n_r_local in {51, 101, 151, 251}, phi at defaults."
            echo "  2. phi grid density scaled via --f-grid, r at n_local=201."
            echo ""
