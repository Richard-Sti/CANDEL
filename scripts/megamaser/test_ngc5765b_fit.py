"""Quick test: fit NGC5765b with proper priors informed by Gao+2016."""
import sys
...
# ---- Config with proper priors ----
# Gao+2016: D=126.3, M_BH=4.55e7, i0=94.5, Omega0=146.7,
#           di/dr=-10.6, dOmega/dr=-3.46, x0=-0.044, y0=-0.100
config = {
    "inference": {
        "num_warmup": 500,
        "num_samples": 500,
    ...
    "io": {
        "fname_output": "results/Maser/test_ngc5765b.hdf5",
    },
}
...
init_values = {
    ...
    'sigma_x_floor': jnp.array(3.0),   # uas
    'sigma_y_floor': jnp.array(3.0),   # uas
...
if 'D_c' in samples:
    ...
    z_cosmo = D_c * 73.0 / 299792.458  # approximate
    D_A = D_c / (1 + z_cosmo)
    print(f"\n  D_A (approx)         = {D_A.mean():10.3f} +/- {D_A.std():8.3f}")
    print(f"  H0 (from D_A, z)    = {(299792.458 * z_cosmo / D_A).mean():10.3f}")

# Compare to Gao+2016
print("\n  Gao+2016 reference:")
print(f"  D_A = 126.3 +/- 11.6 Mpc")
print(f"  M_BH = 4.55e7 +/- 0.40e7 Msun")
print(f"  i0 = 94.5 +/- 0.25 deg")
print(f"  Omega0 = 146.7 +/- 0.125 deg")
print(f"  di/dr = -10.6 +/- 1.13 deg/mas")
print(f"  dOmega/dr = -3.46 +/- 0.43 deg/mas")
